...
class Pipeline:
    ...
    def run(  # type: ignore
        ...
        while queue:
            ...
            if predecessors.isdisjoint(set(queue.keys())):  # only execute if predecessor nodes are executed
                try:
                    ...
                    time1 = time.time()
                    logger.info(f"{config.BENCHMARK_LOG_TAG} request_id = {params['request_id']['id']} Running node `{node_id}` , preprocess time = {time1 - time0}")
                    node_output, stream_id = self.graph.nodes[node_id]["component"]._dispatch_run(**node_input)
                    time2 = time.time()
                    interval = time2 - time1
                    logger.info(f"{config.BENCHMARK_LOG_TAG} request_id = {params['request_id']['id']} Running node `{node_id}`, interval = {interval}")
                ...
                i = 0
                time4 = time.time()
                logger.info(f"{config.BENCHMARK_LOG_TAG} request_id = {params['request_id']['id']} Running node `{node_id}`, postprocess = {time4-time3}")
        ...
        logger.info(f"{config.BENCHMARK_LOG_TAG} request_id = {params['request_id']['id']} pipeline total run time = {end-start}")
        return node_output
